...
function CustomersPage() {
  const { customers, users, addCustomer, updateCustomer, deleteCustomer } = useCrmData();
  const { user } = useAuth();
  ...
  function closeModal() {
    setOpen(false);
    setEditingCustomer(null);
    reset({ category: "Mid Market", owner: "Rohan Iyer", status: "Active" });
  }

  function handleDelete(customer) {
    if (!isAdmin) return;
    if (window.confirm(`Delete customer "${customer.company}"? This data will be deleted.`)) {
    ...
    { key: "owner", header: "Owner" },
    {
      ...
      render: (customer) => (
        <div className="row-actions">
          <Button variant="ghost" size="sm" icon={Pencil} onClick={() => openEdit(customer)}>
            Edit
          </Button>
          {isAdmin && (
            <Button variant="danger" size="sm" icon={Trash2} onClick={() => handleDelete(customer)}>
              Delete
            </Button>
          )}
        </div>
  ...
  return (
    <div className="page-stack">
      ...
      <Modal open={open} title={editingCustomer ? "Edit customer" : "Add customer"} onClose={closeModal}>
          ...
          <div className="form-grid">
            ...
            <FormField label="Status" error={errors.status}>
              <select {...register("status")}>
                {["Active", "Renewal", "At Risk", "Inactive"].map((item) => (
                  <option key={item}>{item}</option>
                ))}
              </select>
            </FormField>
